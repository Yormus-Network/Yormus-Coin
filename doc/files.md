...
The data directory is the default location where the YormusCoin Core files are stored.

1. The default data directory paths for supported platforms are:

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.yormuscoin/`
macOS    | `$HOME/Library/Application Support/YormusCoin/`
Windows  | `%APPDATA%\YormusCoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `yormuscoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual YormusCoin blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `yormuscoin.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `yormuscoind` or `yormuscoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `yormuscoind.pid`        | Stores the process ID (PID) of `yormuscoind` or `yormuscoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `yormuscoind` or `yormuscoin-qt`; can be specified by `-debuglogfile` option
...
`yormuscoin-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by the YormusCoin Core:
