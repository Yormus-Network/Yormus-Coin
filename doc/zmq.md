...
The YormusCoin Core daemon can be configured to act as a trusted "border
router", implementing the yormuscoin wire protocol and relay, making
...
The ZeroMQ feature in YormusCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building yormuscoind:
...
For instance:
    ...
    $ yormuscoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/yormuscoind.tx.raw \
...
These options can also be provided in yormuscoin.conf.
...
From the perspective of yormuscoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into yormuscoind directly. Furthermore, no information is
...
using. YormusCoind appends an up-counting sequence number to each
