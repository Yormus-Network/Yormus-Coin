YormusCoin Core version 0.18.1 is now available from:

  <https://download.yormuscoin.org/yormuscoin-0.18.1/>
...
`/Applications/YormusCoin-Qt` (on Mac) or `yormuscoind`/`yormuscoin-qt` (on
Linux).
...
YormusCoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use YormusCoin Core on unsupported systems.
...
10.10. Additionally, YormusCoin Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:17201:17201` (this is an extra `:17201` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@yormuscoin.org) the YormusCoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 YRMS),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in YRMS per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/yormuscoin-conf.md)
  about the `yormuscoin.conf` file describes how to use it to configure
  YormusCoin Core.

- A new document introduces YormusCoin Core's BIP174 [Partially-Signed
  YormusCoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent YormusCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  YormusCoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when YormusCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of YormusCoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart yormuscoind with the `-deprecatedrpc=generate`
...
- A new `yormuscoin-wallet` tool is now distributed alongside YormusCoin
...
This section describes planned changes to YormusCoin Core that may affect
other YormusCoin software and services.

- Since version 0.16.0, YormusCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with YormusCoin Core 0.20 (expected about
  a year after 0.18), YormusCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the YormusCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in YormusCoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any YormusCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, YormusCoin Core
...
- A sub-project of YormusCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with YormusCoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to YormusCoin Core's own implementation, although entropy
  gathered by YormusCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves YormusCoin Core a
...
- On macOS, YormusCoin Core now opts out of application CPU throttling
...
- #17201 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
