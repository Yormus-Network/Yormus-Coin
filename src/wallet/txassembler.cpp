    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional YRMS outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes YRMS and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::YRMS_TO_YRMS) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_yrms);
        }
    } else {
        // First try to construct a YRMS-to-YRMS transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::YRMS_ONLY;
        ...
        // If YRMS-to-YRMS fails, create a peg-out transaction
