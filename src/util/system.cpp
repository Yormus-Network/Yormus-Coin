...
const char * const BITCOIN_CONF_FILENAME = "yormuscoin.conf";
...
#else
    const char* pszModule = "yormuscoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "YormusCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/YormusCoin";
#else
    // Unix-like
    return pathRet / ".yormuscoin";
#endif
#endif
}
